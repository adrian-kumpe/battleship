  ...
  describe('config.ts', () => {
    describe('AVAILABLE_MARKERS', () => {
      test('should have four markers for the left grid', () => {
        expect(AVAILABLE_MARKERS.filter((m) => m.role === MARKER_ROLE.CORNER_LEFT_GRID).length).toBe(4);
      });

      test('should have four markers for the right grid', () => {
        expect(AVAILABLE_MARKERS.filter((m) => m.role === MARKER_ROLE.CORNER_RIGHT_GRID).length).toBe(4);
      });
    });
